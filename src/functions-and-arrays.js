...
function sum(array) {
  ...
  array.forEach(function (element) {
    ...
    else if (typeof element === "boolean") sum += element ? 1 : 0;
    else if (typeof element === "number") sum += element;
  ...
  for (let i = 0; i < matrix.length; ++i) {
    for (let j = 0; j < matrix[i].length; ++j) {
      if (i < matrix.length - 4) {
        ...
        if (prodAux > product) product = prodAux;
      }
...
function greatestProductDiagonal(matrix) {
  ...
  for (let i = 0; i < matrix.length; ++i) {
    for (let j = 0; j < matrix[i].length; ++j) {
      if (i < matrix.length - 4) {
        //como revisamos las diagonales de todas las posiciones de la matrix, no hace falta mirar todo el rato las 4 diagonales. nos bastará con mirar solo dos (arriba derecha y abajo derecha)
        if (j < matrix[i].length - 4) {
          let prodAux =
            matrix[i][j] *
            matrix[i + 1][j + 1] *
